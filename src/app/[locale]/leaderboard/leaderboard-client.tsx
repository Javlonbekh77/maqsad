...
export default function LeaderboardClient() {
  ...
  const [topSilverCoinUsers, setTopSilverCoinUsers] = useState<User[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoadingData(true);
    try {
      const { topUsers, topGroups, topSilverCoinUsers } = await getLeaderboardData();
      setTopUsers(topUsers);
      setTopGroups(topGroups);
      setTopSilverCoinUsers(topSilverCoinUsers);
    } catch (error) {
